  		...
  		<form class="form-signin" id="searchList" name="searchList" method="post" action="searchList.do">
      		<div class="box box-default">
        		...
        		<div class="box-header with-border">
          			<h3 class="box-title"><spring:message code="common.wd.상품관리" /> > <spring:message code="common.wd.상품리스트" /></h3>
        		</div>
        		<!-- /.box-header -->
        		<div class="box-body">
          			<div class="row">
            			...
            			<div class="col-md-6">
	              			<div class="form-group">
								<div class="row">
									...
									<div class="col-md-10"><input type="text" id="sch_prd_name" name="sch_prd_name" class="form-control" style="width: 100%;" value="${paramMap.sch_prd_name}"/></div>
								</div>
	              			</div>
            			</div>
            			<!-- /.col -->
              			...
              			<table id="example2" class="table table-bordered table-hover">
                			<thead>
                				<tr>
                  					...
                  					<th width="10%"><spring:message code="common.wd.상품명" /></th>
                  					<th width="10%"><spring:message code="common.wd.회원아이디" /></th>
                			...
                			<tbody>
                				<c:forEach items="${pageHelper.list}" var="list">
	                				<tr>
	                  					<td>${list.prd_mst_seq}</td>
	                  					<td><a href="updateForm.do?curr_page=${pageHelper.currPage}&prd_mst_seq=${list.prd_mst_seq}">${list.prd_name}</a></td>
	                  					<td>${list.member_id}</td>
	                  					<td>${list.prd_in_price}</td>
	                  					<td>${list.prd_sl_price}</td>
	                  					<td><cf:common_code_name code_group_id="PRD_STATUS" code_id="${list.prd_status}" /></td>
	                  					<td>${list.regdate}</td>
	                  					<td>${list.uptdate}</td>
	                				</tr>
                				</c:forEach>
