			...
			if (tbl_common_codeList.size() > 0) {
				...
				for (int i = 0; tbl_common_codeList.size() > i; i++) {
					...
					if ((tag_value != null) && !tag_value.equals("") && tag_value.equals(vw_common_code.getCode_id())) {
						sb.append("<input type=\"radio\" id=\"" + tag_id + "\" name=\"" + tag_name + "\" value=\"" + vw_common_code.getCode_id() + "\" {class} {style} {onclick} checked>");
						sb.append(vw_common_code.getCode_name() + "</input>");
					} else {
						sb.append("<input type=\"radio\" id=\"" + tag_id + "\" name=\"" + tag_name + "\" value=\"" + vw_common_code.getCode_id() + "\" {class} {style} {onclick}>");
						sb.append(vw_common_code.getCode_name() + "</input>");
					}
				}
